...
///READ TEPX number of Clusters per event for pu=200
  ...
  for(long d=0;d<8;d++){
    TH2F* H2 = (TH2F*)FTEPX_clu.Get(TString("BRIL_IT_Analysis/TEPX/Clusters/Number of clusters for Disk ")+disk_clu[d].c_str());
  ...
  for(long d=0;d<8;d++){
    TH2F* H2 = (TH2F*)FTEPX_2x.Get(TString("BRIL_IT_Analysis/TEPX/2xCoincidences/Number of real 2x Coincidences in R for Disk ")+disk_2x[d].c_str());
    TProfile* P=H2->ProfileX();
    for(long r=1;r<=5;r++){
...
///READ TEPX number of 2x Coincidences per event for pu=200
  ...
  TFile FTEPX_2xnew("/home/ashish/Desktop/TEPX_0.01new/samples_17Feb2020/2023D42PU200.root","r");
  std::vector<string> disk_2xnew={"-4","-3","-2","-1","1","2","3","4"};
  for(long d=0;d<8;d++){
    TH2F* H2 = (TH2F*)FTEPX_2xnew.Get(TString("BRIL_IT_Analysis/TEPX/2xCoincidences/Number of real 2x Coincidences in R for Disk ")+disk_2xnew[d].c_str());
    TProfile* P=H2->ProfileX();
    for(long r=1;r<=5;r++){
