...
private:
  ...
  const SiPixelCluster* findCoincidence2x(DetId, Global3DPoint, bool, unsigned int&, edmNew::DetSet<SiPixelCluster>::const_iterator);
  ...
  for (typename edmNew::DetSetVector<SiPixelCluster>::const_iterator DSVit = clusters->begin(); DSVit != clusters->end(); DSVit++) {
	...
	if (m_docoincidence) {
	  ...
	  unsigned int coincidenceId;
	    const SiPixelCluster* found2xcoincidencecluster = this->findCoincidence2x(detId, globalPosClu1, true, coincidenceId, cluit1);
...
const SiPixelCluster* Ashish2xCoincidence5::findCoincidence2x(DetId thedetid, Global3DPoint globalPosClu1, bool isTEPX, unsigned int& foundDetId, edmNew::DetSet<SiPixelCluster>::const_iterator cluit1) {
  ...
  for (edmNew::DetSet<SiPixelCluster>::const_iterator cluit2 = theit->begin(); cluit2 != theit->end(); cluit2++) {
    ...
    if (fabs(globalPosClu1.x() - globalPosClu2.x()) < m_dx
        ...
        && fabs(globalPosClu1.z() - globalPosClu2.z()) < m_dz && thering == 1) {

   
      // if((phi2-phi1) < m_dphi_ring1 && (r2-r1) < m_dr_ring1 && thering == 1) {
